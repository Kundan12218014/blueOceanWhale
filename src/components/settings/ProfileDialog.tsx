...
export default function ProfileDialog({ isOpen, onClose }: ProfileDialogProps) {
  ...
  const [newPhotoURL, setNewPhotoURL] = useState(''); // New state for URL input
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        const photoURL = await updateUserProfile(displayName, file);
        setPhotoURL(photoURL || '');
        setNewPhotoURL(''); // Clear URL input after upload
      } catch (error) {
